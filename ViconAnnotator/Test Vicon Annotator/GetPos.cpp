...
void GetPos::parsedata2(const string& filename, int numFrames) {
	map<int, string> markers;
	int i = 0;
	int k = 0;
	int colNum = 1;
	g_vertex_buffer_data = new float[numFrames * 17 * 3];
	string s;
	ifstream f(filename.c_str());
	vector<Marker> MarkerList;
	while (getline(f, s))  // get the rows of the csv file
		...
		while (getline(iss, s, ','))	// get each cell in the row
		{
			...
			if (fieldvalue == "" && colNum < 3) {  // skip first two cells of each row
				...
				if (markers[currColNum] == "") {	// skip values until next marker
					...
					int currindex = (currColNum - 3);
					*(g_vertex_buffer_data++) = atof(currY.c_str())/1000 - 0.5;
					*(g_vertex_buffer_data++) = atof(currX.c_str()) / 1000 - 0.5;
					*(g_vertex_buffer_data++) = atof(currZ.c_str())/1000 - 0.5;
